...
class TestCompleteGameFlow:
    ...
    def test_game_with_no_goals(self, client: TestClient):
        ...
        for i, player in enumerate(players):
            team_id = game["home_team_id"] if i == 0 else game["away_team_id"]
...
class TestDataIntegrity:
    ...
    def test_player_stats_update_after_goal(self, client: TestClient):
        """Test that player stats are correctly updated after scoring"""
        # Setup
        stadium = client.post("/api/stadiums", json={
            "name": "Stats Stadium", "address": "Stats St"
        }).json()
        
        player = client.post("/api/players", json={
            "name": "Scorer", "nickname": "SC"
        }).json()
        ...
        initial_stats = client.get(f"/api/players/{player['id']}").json()
        
        # Start game and record goal
        client.put(f"/api/games/{game['id']}/start")
        client.post(f"/api/games/{game['id']}/goals", json={
            "game_id": game["id"],
            "team_id": game["home_team_id"],
            "scorer_id": player["id"],
            "minute": 10
        ...
        updated_stats = client.get(f"/api/players/{player['id']}").json()
        
        # Verify stats changed (this depends on your stats calculation)
        # You may need to adjust this based on your actual stats structure
        player_games = client.get(f"/api/players/{player['id']}/games").json()
        assert len(player_games) == 1
